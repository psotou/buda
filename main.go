...
func main() {
	...
	lastPrice, _ := strconv.ParseFloat(market.LastPrice[0], 64)
	minVenta, _ := strconv.ParseFloat(market.MinAsk[0], 64)
	maxCompra, _ := strconv.ParseFloat(market.MaxBid[0], 64)
	...
	fmt.Printf("Precio última orden ejecutada  %.1f CLP\n", lastPrice)
	fmt.Printf("Menor precio de venta          %.1f CLP\n", minVenta)
	fmt.Printf("Máximo precio de compra        %.1f CLP\n", maxCompra)
	fmt.Printf("SPREAD                         %.1f CLP\n", spread)
	fmt.Printf("Variación últimas 24h          %2.2f %%\n", oneDayVariationPercent)
	fmt.Printf("Variación últimos 7d           %2.2f %%\n", sevenDayVariationPercent)
	fmt.Printf("Volumen venta últimas 24h      %3.2f %s (%2.1f %%)\n", volVenta, criptomoneda, porcentajeVolVenta)
	fmt.Printf("Volumen compra últimas 24h     %3.2f %s (%2.1f %%)\n", volCompra, criptomoneda, porcentajeVolCompra)
}
